...
| 参数 | 子命令 | 说明 |
|------|--------|------|
| `route` | `/cost-route` | Smart model routing |
| `compress` | `/cost-compress` | Context compression |
| `heartbeat` | `/cost-heartbeat` | Heartbeat optimization |
| `report` | `/cost-report` | Usage report |
| `config` | `/cost-config` | Config generation |
| `slim` | `/cost-slim` | Audit & slim down SOUL.md, AGENTS.md, MEMORY.md |
| `screenshot` | `/cost-screenshot` | Optimize screenshot resolution for vision tasks |
| `loops` | `/cost-loops` | Optimize agent loop settings to reduce overhead |
| (no argument) | `/cost-report` | Default: show usage report |
...
## 6. `/cost-slim` — Context File Audit & Optimization

The single biggest hidden cost in OpenClaw: **SOUL.md, AGENTS.md, and MEMORY.md are injected into EVERY message.** A 5,000-token SOUL.md × 100 messages/day = 500,000 wasted input tokens/day (~$7.50 on Opus).

### Step 1: Measure current context overhead

Scan and count tokens in all context files:
...
for f in SOUL.md AGENTS.md MEMORY.md USER.md HEARTBEAT.md; do
  if [ -f "$f" ]; then
    chars=$(wc -c < "$f")
    est_tokens=$((chars * 100 / 400))  # rough: 1 token ≈ 4 chars for English, 2 chars for Chinese
    echo "$f: $chars chars ≈ $est_tokens tokens"
  fi
done
...
for f in CLAUDE.md; do
  if [ -f "$f" ]; then
    chars=$(wc -c < "$f")
    est_tokens=$((chars * 100 / 400))
    echo "$f: $chars chars ≈ $est_tokens tokens"
  fi
done
```

### Step 2: Apply token budgets

| File | Target Budget | Why |
|------|--------------|-----|
| SOUL.md | < 1,000 tokens (~2,000 chars EN / ~1,000 chars CN) | Loaded on EVERY message. Every token here is multiplied by total messages |
| AGENTS.md | < 800 tokens | Same reason — injected per message |
| MEMORY.md | < 3,000 tokens | Loaded per session. Don't dump project docs here |
| USER.md | < 500 tokens | Brief user context only |
| HEARTBEAT.md | < 300 tokens | Minimal heartbeat instructions |
| **Total overhead** | **< 5,600 tokens** | Target: keep combined context under 6K |

### Step 3: Slim down SOUL.md

Read the current SOUL.md and rewrite it following these rules:

**Rules for an efficient SOUL.md:**

1. **No redundant instructions** — If the model already does it by default, don't say it
   - ❌ "You are a helpful assistant" (it already is)
   - ❌ "Write clean code" (it already tries to)
   - ✅ "Reply in Chinese" (non-default behavior)

2. **No long lists when a short rule works**
   - ❌ "You are proficient in React, Vue, Angular, Svelte, Next.js, Nuxt, Remix, Astro, SolidJS..."
   - ✅ "Full-stack engineer. Adapt to the project's framework."

3. **No examples unless critical** — Examples bloat SOUL.md. Put them in reference files loaded on demand.

4. **Merge overlapping rules**
   - ❌ "Be concise." + "Don't be verbose." + "Keep responses short." (3 rules, same meaning)
   - ✅ "Be concise." (1 rule)

5. **Use bullet points, not paragraphs** — Bullets are shorter and models parse them better.

### Step 4: Slim down AGENTS.md

- Remove agent definitions you haven't used in the past week
- Reduce each agent description to 1 line (name + one-sentence capability)
- Move detailed agent prompts to separate files loaded on demand

### Step 5: Slim down MEMORY.md

- Remove entries older than 2 weeks unless explicitly marked as permanent
- Keep only index pointers (file paths), not inline content
- Target: each memory entry = 1 line in the index

### Step 6: Output optimization report
...
| File | Tokens | Status |
|------|--------|--------|
| SOUL.md | {before_tokens} | {over_budget ? "🔴 OVER" : "🟢 OK"} |
| AGENTS.md | {before_tokens} | ... |
| MEMORY.md | {before_tokens} | ... |
| **Total** | **{total_before}** | |

### After
| File | Tokens | Saved |
|------|--------|-------|
| SOUL.md | {after_tokens} | {saved} ({pct}%) |
| AGENTS.md | {after_tokens} | {saved} ({pct}%) |
| MEMORY.md | {after_tokens} | {saved} ({pct}%) |
| **Total** | **{total_after}** | **{total_saved} ({total_pct}%)** |

### Monthly savings estimate
- Messages/day: ~{n}
- Token reduction per message: {saved_per_msg}
- Monthly savings: **${monthly_savings}** (at {model} pricing)

### Optimized SOUL.md preview
{show the proposed slim version for user confirmation}
```

Confirm with user before applying changes. Back up originals as `SOUL.md.bak.{timestamp}`.

---

## 7. `/cost-screenshot` — Screenshot Resolution Optimization

When using browser automation (Playwright, Puppeteer, idb, ADB) for testing, screenshots are a **massive hidden token cost**. A 4K screenshot ≈ 1,600 tokens. An optimized screenshot ≈ 300 tokens.

### Step 1: Detect current screenshot settings

```bash
# Check screen resolution
system_profiler SPDisplaysDataType 2>/dev/null | grep Resolution || xrandr 2>/dev/null | grep '*'

# Check if browser automation is configured
ls playwright.config.* puppeteer.config.* 2>/dev/null
```

### Step 2: Calculate optimal resolution

The goal: **find the minimum resolution where OCR/UI recognition still works reliably.**

| Use Case | Recommended Resolution | Approx Size | Approx Tokens |
|----------|----------------------|-------------|---------------|
| Full 4K (no optimization) | 3840 × 2160 | ~2 MB | ~1,600 |
| UI element detection | 1280 × 720 | ~200 KB | ~500 |
| Text OCR (general) | 1024 × 768 | ~120 KB | ~400 |
| Text OCR (minimum viable) | 800 × 600 | ~80 KB | ~300 |
| Layout-only checks | 640 × 480 | ~40 KB | ~200 |

**Rule of thumb: 1280×720 is the sweet spot** — enough for both OCR and UI element detection.
...
use: {
  ...
  screenshot: {
    quality: 60,  // JPEG quality (default 100)
    type: 'jpeg',  // JPEG is smaller than PNG
  },
}
```

For OpenClaw browser tool:
```bash
# Before screenshot, resize viewport
mcp__playwright__browser_resize --width 1280 --height 720
# Or for Claude-in-Chrome
mcp__claude-in-chrome__resize_window --width 1280 --height 720
```

For manual screenshot compression:
```bash
# Using ImageMagick (if available)
convert screenshot.png -resize 1280x720 -quality 60 screenshot_optimized.jpg

# Using sips (macOS built-in)
sips -Z 1280 screenshot.png --out screenshot_optimized.jpg
```

### Step 4: Output savings estimate
...
|--------|--------|-------|---------|
| Resolution | {original} | {optimized} | — |
| Avg file size | {before_size} | {after_size} | {pct}% |
| Tokens per image | ~{before_tokens} | ~{after_tokens} | {token_pct}% |
| Screenshots/session | ~{count} | ~{count} | — |
| **Tokens/session** | **{total_before}** | **{total_after}** | **{total_pct}%** |
| **Cost/session** | **${cost_before}** | **${cost_after}** | **${cost_saved}** |

Recommendation applied: viewport set to {width}×{height}, JPEG quality {quality}%.
```

---

## 8. `/cost-loops` — Agent Loop Optimization

In harness mode and team mode, **each agent loop iteration re-transmits the full conversation context.** Reducing iterations and increasing work per iteration directly cuts token overhead.

### Step 1: Analyze current loop patterns

Check for common loop-heavy patterns:

```bash
# Check if openclaw.json has loop/iteration settings
cat openclaw.json 2>/dev/null | python3 -c "
import sys, json
try:
    cfg = json.load(sys.stdin)
    print(json.dumps(cfg.get('agents', {}), indent=2))
except: print('No agent config found')
"
```

### Step 2: Apply loop optimization rules

| Pattern | Problem | Optimized |
|---------|---------|-----------|
| Test 10 pages, 1 per loop | 10× context re-transmission | Test 3-4 pages per loop, 3 iterations |
| Retry on failure, no limit | Infinite loop risk, burns tokens | Set `max_iterations: 5`, then stop or escalate |
| Agent checks status every loop | Redundant reads each iteration | Batch status checks, report once at end |
| Sub-agent per file | N agents × full context each | 1 agent processes multiple files sequentially |

### Step 3: Generate recommended settings
...
```json
{
  "agents": {
    "max_iterations": 5,
    "batch_size": "auto",
    "on_limit_reached": "summarize_and_stop",
    "loop_optimization": {
      "combine_small_tasks": true,
      "min_work_per_iteration": 3,
...
### Step 4: Calculate overhead reduction

```markdown
## 🔄 Loop Optimization Report

### Current pattern
- Avg iterations per task: {current_iterations}
- Context size per iteration: ~{context_tokens} tokens
- Context overhead: {iterations} × {context_tokens} = **{total_overhead} tokens**

### Optimized pattern
- Avg iterations per task: {optimized_iterations}
- Work per iteration: {work_per_iteration}× more
- Context overhead: {opt_iterations} × {context_tokens} = **{opt_overhead} tokens**
- **Overhead reduction: {reduction_pct}%**

### Monthly impact
- Tasks/day: ~{tasks}
- Monthly savings from loop optimization: **${monthly_savings}**
```
