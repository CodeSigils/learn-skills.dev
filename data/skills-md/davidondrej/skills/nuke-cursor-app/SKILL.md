...
- The metrics collector OWNS the metrics database. Read it
...
```bash
DB="<path-to-metrics-database>"
LOG_DIR="<path-to-nuke-logs-directory>"
mkdir -p "$LOG_DIR"
...
ps -axo rss,comm | awk '/\/Applications\/Cursor.app/ {n++; s+=$1} END {printf "%d processes, %.1f GB\n", n, s/1048576}'

# Last 30 min from the collector (read-only)
sqlite3 -readonly "$DB" "<query for recent process samples>"
sqlite3 -readonly "$DB" "<query for recent events>"
```
...
> using 2.1 GB. Snapshot saved to <nuke-logs-dir>/2026-08-14-1832.md.
