...
- **Consequence, not pattern.** Every finding says who is affected and how. "This query isn't scoped by user" is a pattern; "any signed-in account can read every other account's invoices by changing the id in the URL — `app/src/app/invoices/[id].tsx:14`" is a finding. If the path from the outside world to the flaw can't be described, it isn't one yet.
...
- **The stack.** `package.json` in both packages, the Expo app's router and UI conventions, the Worker's framework, its bindings, and the auth library. This skill is written for what `start-an-app` builds — an Expo app (expo-router, TypeScript, Tabler icons, Clerk, React Query) talking to a Cloudflare Worker (Hono) with D1, R2, KV, Workers AI, cron/Queues and Email Service, Clerk JWTs verified in the Worker. **Where it meets something else, say so plainly in the report and say which checks that weakened.** A review that pretends to full coverage over a stack it doesn't know is worse than a narrower one that says where its edges are.
- **Which branches exist.** Sign-in (Clerk), email (Email Service), uploads (R2), payments, AI (Workers AI / AI Gateway), background jobs (cron/Queues), agent access over MCP, documentation, legal pages, a consent banner. Each one turns on a section of a lens and, more usefully, turns the others *off*.
- **Whether the app is meant to be found.** `robots.txt`, `sitemap.xml` and `llms.txt` in the Expo app's public directory, and the metadata in `app.json` and the root layout. This decides the entire shape of the discoverability lens, and getting it backwards produces a report telling someone to optimise an internal tool.
...
`references/evidence.md` has the commands: the route inventory for the Expo app and the Worker, whether either one runs (`npx expo start --web`, `wrangler dev`), the probe sweep against the local Worker, and what to capture. Three lenses read the same capture, so it is gathered once and it has to be complete before any of them start.

> **Hard rule: no secret enters the evidence pack.** It is handed to subagents and quoted in a report. Variable *names* from `.env` and `.dev.vars`, never values; `git diff` and served HTML get read for leaks, never pasted wholesale. A review that leaks the key it was checking on is the only way this skill can do real damage.
...
| Discoverability | `references/seo.md` | the exported web app's `robots.txt`, `sitemap.xml`, `llms.txt` and metadata — correct and current, or not |
...
- **A fix that needs a schema change goes through a new D1 migration (`wrangler d1 migrations create`), a read of the generated SQL, then `wrangler d1 migrations apply`.** Never hand-apply DDL, never an edit to a migration already applied.
