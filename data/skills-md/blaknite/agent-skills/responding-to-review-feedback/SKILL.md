...
4. Commit the change with a clear message referencing the feedback
5. Push the commit to the PR branch

Push after each fix, not in a batch at the end. This keeps replies honest: by the time you acknowledge a suggestion, the fix is already on the branch the reviewer can see.

If the commit or push fails (tests, hooks, conflicts), stop and resolve it before replying. Never post an acknowledgement for code that isn't on the remote.
...
For replies acknowledging an implemented suggestion, the commit must already be pushed before you draft the reply. For replies that decline a suggestion or answer a question, there's nothing to push—draft immediately.
...
**You:** "Implementing now... Done. Committed and pushed as `abc1234`."

**You:**
> **Draft reply to @alice (comment 12345):**
>
> > Good call, switched to `present?`. Pushed in abc1234.
>
> Want me to post this, or would you like to edit it?
