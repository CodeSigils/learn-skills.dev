...
**Default model on initialization**: if no model is configured yet, initialize this skill with **Nano Banana Pro** (`CHATBOT_GEMINI_3_PRO_IMAGE_PREVIEW`) and tell the user that it is now the default. Also remind them they can switch models anytime later.
...
2. Initialize local defaults with **Nano Banana Pro** (`CHATBOT_GEMINI_3_PRO_IMAGE_PREVIEW`) by writing `EXTEND.md`
3. Tell the user that the default model is now **Nano Banana Pro**, and explicitly remind them they can switch anytime later if they need another model
...
9. **Default to Nano Banana Pro unless the user asks otherwise.** When no default model is configured yet, initialize `CHATBOT_GEMINI_3_PRO_IMAGE_PREVIEW` (Nano Banana Pro), tell the user that it is now the default for this skill, and explicitly remind them they can switch any time if another model fits better.
