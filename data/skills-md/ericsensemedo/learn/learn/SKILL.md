...
If an Obsidian MCP server is available (e.g., `aaronsb/obsidian-mcp-plugin` for
graph-aware access, or `StevenStavrakis/obsidian-mcp` for filesystem access):

1. When a gap is detected and the concept exists as a vault page, pull context
   from the vault to enrich the explanation
2. When the user learns something new during a coding session, offer to file it
   as a vault note (compounding knowledge, Karpathy-style)
3. Use vault graph traversal to find related concepts the user might also need
4. During lint/review passes, cross-reference `LEARNING.md` concepts against
   vault pages to find gaps where knowledge exists in the vault but the user
   hasn't learned it yet

### Without MCP

Even without an MCP server, if the vault path is known, the agent can read
markdown files directly from the vault directory. Obsidian vaults are just
folders of `.md` files — no special tooling required for basic read access.
