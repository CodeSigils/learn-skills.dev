---
name: cs-issue
description: 修 bug 的子流程入口。默认快速定位并留下 fix-note；根因不明、影响面大或用户要求完整留档时才补 report / analysis。触发：用户说"修 bug"、"有个问题"、"修复 XX"。只做路由。
---
...
issue 工作流默认不塞完整缓冲。根因明确的小 bug 直接走快速通道：

```
读代码 → 告知根因 + 修复方案 → 用户确认 → 修复验证 → fix-note
```

只有根因不明 / 多候选 / 跨模块影响时，才走标准缓冲：
...
| 当前状态 | 触发哪个子技能 |
|---|---|
| 刚发现问题，没有任何文件，且根因可快速确认 | 直接走快速通道：读代码后进入 `cs-issue-fix` |
| 刚发现问题，但根因不明 / 影响面大 / 用户要完整记录 | `cs-issue-report` |
