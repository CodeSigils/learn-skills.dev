...
When the decision tree is resolved, the glossary/ADRs are updated, and we're aligned, **write the agreed plan to `PLAN_FILE`** (resolve it first — see Act 2's Tunables; default `PLAN.md`; use the canonical terms from `CONTEXT.md`), then run Act 2:
...
codex exec resume "$THREAD_ID" -c sandbox_mode="read-only" --json \
  ...
  "I revised the plan. Re-review <PLAN_FILE> — check prior findings + flag anything new. End with VERDICT: APPROVED or VERDICT: REVISE." \
...
- **Act 3 (optional):** user picks Codex → invoke the `codex-build` skill with `SPEC_FILE=<PLAN_FILE>` and the same `LOG_FILE`. Roles flip: Codex writes with full access, Claude reviews the diff + runs the proof; build rounds append to the same log.
