---
name: init-playwright-architecture
description: Add the Playwright architecture rule to a project's `.claude/rules/` folder. Installs Page Object conventions, custom fixtures, auth setup with storageState, directory structure, config best practices, and anti-patterns. Use when adding Playwright to a new project or refreshing the architecture rule in an existing one.
compatibility: Designed for Claude Code
disable-model-invocation: true
---

# Initialize Playwright Architecture Rule

Write the Playwright architecture rule to `.claude/rules/playwright-architecture.md` in the current project. The rule auto-loads when Claude touches files matching `tests/**`, `playwright-utils/**`, or `playwright.config.ts`.
...
3. **Check for existing file**:
   - **Not present** → write the content from the `<!-- RULES_START -->` block below, then report `Created .claude/rules/playwright-architecture.md`.
   ...
   - **Present and different** → use `AskUserQuestion` with options: **Overwrite / Show diff / Skip**.
     - **Overwrite** → write content, report `Updated .claude/rules/playwright-architecture.md`.
     - **Show diff** → run `git diff --no-index .claude/rules/playwright-architecture.md <temp-file-with-new-content>`, then re-prompt **Overwrite / Skip**.
...
- This skill is **self-contained**: the rule lives inside `SKILL.md`, so it works on any machine, even one without other Playwright skills installed.
- To update the rule across projects, update this `SKILL.md` once, then re-run `/init-playwright-architecture` in each project.
