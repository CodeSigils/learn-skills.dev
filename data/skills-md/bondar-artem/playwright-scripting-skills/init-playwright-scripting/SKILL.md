---
name: init-playwright-scripting
description: Add the Playwright scripting rule to a project's `.claude/rules/` folder. Installs E2E test authoring conventions — locators, assertions, waiting, test structure, naming, code style, actions, and form interactions. Use when adding Playwright to a new project or refreshing the scripting rule in an existing one.
compatibility: Designed for Claude Code
disable-model-invocation: true
---

# Initialize Playwright Scripting Rule

Write the Playwright scripting rule to `.claude/rules/playwright-scripting.md` in the current project. The rule auto-loads when Claude touches files matching `tests/**` or `playwright-utils/**`.
...
1. **Verify project context** — cwd must contain `package.json`. Otherwise stop and tell the user.
2. **Ensure `.claude/rules/` exists** — `mkdir -p .claude/rules`.
3. **Check for existing file**:
   - **Not present** → write the content from the `<!-- RULES_START -->` block below, then report `Created .claude/rules/playwright-scripting.md`.
   ...
   - **Present and different** → use `AskUserQuestion` with options: **Overwrite / Show diff / Skip**.
     - **Overwrite** → write content, report `Updated .claude/rules/playwright-scripting.md`.
     - **Show diff** → run `git diff --no-index .claude/rules/playwright-scripting.md <temp-file-with-new-content>`, then re-prompt **Overwrite / Skip**.
...
- To update the rule across projects, update this `SKILL.md` once, then re-run `/init-playwright-scripting` in each project.
