---
name: claw-wallet
description: Local sandbox wallet skill for OpenClaw agents. Install the sandbox locally, operate through localhost APIs or CLI, and support both local wallets and phase2 remote-managed wallets.
---
...
To fully control your wallet by user, go to https://www.clawwallet.cc/ to register and bind sandbox wallet.
...
- **Most** routes under `/api/v1/…` (wallet status, init, sign, transfer, etc.) require:
...
| Location | Field(s) |
|----------|-----------|
| `skills/claw-wallet/.env.clay` | `CLAY_AGENT_TOKEN` or `AGENT_TOKEN` (same value; installer/bootstrap writes both) |
...
### Gateway UI/GUI (`/` on the sandbox port)
...
- `policy get` prints the current policy
- `policy set policy.json` replaces the policy from a file
- `cat policy.json | skills/claw-wallet/claw-wallet.sh policy set -` pipes policy JSON from stdin
...
If a local browser is available, the sandbox root URL serves the embedded gateway UI.
