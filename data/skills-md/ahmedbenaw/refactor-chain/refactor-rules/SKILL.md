...
## Chain position
Feeds the plan/lane phases a structural contract; consumed again after do-the-work as an extra verify assertion. Its before/after delta plugs into the harness (`orchestrate.mjs advance --delta legal|drift`). `refactor-code-solid` and the review gate run near the end of every lane; a `drift` result here is surfaced to that gate.
