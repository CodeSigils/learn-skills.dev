...
## Chain position
Runs in the **secure** phase after do-the-work, once a lane (Java/Web/UI/SOLID) has produced changes. Its report feeds the **review gate**, which runs near the end of every lane alongside `refactor-code-solid`. Blockers pause the chain for a human decision before ship. If the repo contains auth code, `refactor-auth-hardening` runs in this same secure phase to harden it; this skill flags auth *risks*, that skill *fixes* auth patterns.
