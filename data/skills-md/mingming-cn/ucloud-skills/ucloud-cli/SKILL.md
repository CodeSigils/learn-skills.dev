...
**安装方法**
- macOS：按架构从 `https://ucloud-infra.cn-bj.ufileos.com/cli/darwin_{amd64,arm64}/ucloud` 下载二进制文件，安装到 `$PATH` 中的目录。
- Linux：按架构从 `https://ucloud-infra.cn-bj.ufileos.com/cli/linux_{amd64,arm64}/ucloud` 下载二进制文件，安装到 `$PATH` 中的目录。
- Windows：从 `https://ucloud-infra.cn-bj.ufileos.com/cli/windows_amd64/ucloud.exe` 下载二进制文件，并放到 `PATH` 中的目录。
...
默认使用产品专用 CLI 子命令，因为它们更贴近用户意图、参数语义更清晰，也更符合官方 CLI 的高层操作入口：

```bash
ucloud --profile prod-account uhost describe --Region cn-bj2
ucloud --profile prod-account eip list --Region cn-bj2
ucloud --profile prod-account ulb describe --Region cn-bj2 --ULBId ulb-xxxx
```

如果用户没有指定 profile，省略 `--profile` 让 CLI 使用本地激活配置。
...
```bash
ucloud --profile prod-account api --Action DescribeInstance --Region cn-bj2
```
...
```bash
UCLOUD_CLI_DEBUG=on ucloud --profile prod-account uhost describe --Region cn-bj2
```
