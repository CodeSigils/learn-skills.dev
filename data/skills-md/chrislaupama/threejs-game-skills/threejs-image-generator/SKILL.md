...
Provider: the OpenAI Image API (gpt-image models).
...
If the probe says `OPENAI_API_KEY=SET` but the script sees no key, run through a shell that sources the user's profile:
...
```bash
uv run ~/.codex/skills/threejs-image-generator/scripts/generate_image.py --prompt "your image description" --filename assets/concepts/output.png --quality high
```

Claude install path:

```bash
uv run ~/.claude/skills/threejs-image-generator/scripts/generate_image.py --prompt "your image description" --filename assets/concepts/output.png --quality high
```
...
- `--size`: `1024x1024` (default via `auto` for square assets), `1536x1024` for wide sky/background plates, `1024x1536` for tall/portrait sheets.
- `--quality`: `low` for quick drafts, `medium` for iteration, `high` (default) for production references, image-to-3D inputs, textures, and UI art.
- `--background transparent`: alpha-preserving PNG output for icons, decals, logos, and GUI elements.
- `--input-image` (repeatable): edit or combine existing images.
...
Logo/icon/UI art (pair with `--background transparent`):
...
Sky/background (pair with `--size 1536x1024`):
