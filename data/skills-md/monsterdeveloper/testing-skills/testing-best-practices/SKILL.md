...
description: >-
  Write high-quality automated tests for JS/TS applications.
  Use when: writing unit, integration, or component tests, testing
  React/Vue components, working with Vitest or similar test runners,
  mocking APIs or dependencies, debugging flaky tests, improving
...
> Practical check: after writing a test, temporarily break the behavior it tests to confirm it actually fails.
...
Use `using` (disposable objects) for automatic cleanup. Requires TypeScript ≥ 5.2 (or the `disposablestack` polyfill).
...
Don't mock: Internal logic you're testing. Overmocking leaves nothing actually being tested.
...
```js
// BAD: false positive — passes immediately even if notification appears later
await waitFor(() => expect(notification).not.toBeInTheDocument())

// GOOD: assert the positive case fails instead
...
When test setup is **more complex than the code being tested**, that's a code smell — revisit the code design, not the test.

Don't modify a function's public API just to make mocking easier:

```js
// BAD: changed signature for tests, not for real consumers
export function query(open: (address: string) => void) { open() }

// GOOD: keep the real API; mock the dependency in test setup instead
export function query(address: string) { sql.open(address) }
```

Invest in shared test utilities to absorb setup complexity rather than spreading it across tests.
...
**Tool selection:**
- Vitest (Node.js): unit and integration tests
- Vitest Browser Mode: component tests
- Playwright (`@playwright/test`): end-to-end tests
- Bun test runner: when working with Bun runtime
...
- [references/vitest-patterns.md](references/vitest-patterns.md) — when configuring Vitest, setting up Browser Mode, writing async tests, or managing mocks
- [references/component-testing.md](references/component-testing.md) — when testing React/Vue components, querying elements, handling user interactions, or mocking APIs with MSW
