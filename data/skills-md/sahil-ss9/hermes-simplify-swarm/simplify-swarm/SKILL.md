...
9. **Removing the wrong copy of a duplicate.** When multiple agents flag a duplicate method/function/block, always verify WHICH copy to keep before cutting. The first definition may be correctly placed (in the right section, near related code) while the second is a stray copy-paste. Before applying the patch: read both copies, note their positions in the file, confirm which one belongs, then explicitly target the stray. Cutting the well-placed copy and leaving the stray creates a worse outcome than the original duplication.

## Verification Checklist

- [ ] Scope correctly identified (git diff, not full repo)
- [ ] Dispatch status block printed BEFORE delegate_task (user knows what's running and for how long)
- [ ] All three agents dispatched in parallel (not sequentially)
- [ ] Approval gate honoured — no edits before the user replied (unless auto_apply/pre-authorised)
