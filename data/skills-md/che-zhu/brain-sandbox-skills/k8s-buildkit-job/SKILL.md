...
description: Run a temporary BuildKit daemon in the active Kubernetes sandbox namespace, then use sandbox-local buildctl to send the local build context and push a GHCR image. Use after brain-github-deploy writes .sealos/build-request.json, or when creating, debugging, or inspecting sandbox BuildKit builds based on moby/buildkit buildkitd and buildctl.
compatibility: Requires kubectl access through the sandbox-provided kubeconfig and service account. The sandbox identity must be able to create Jobs, Services, and Secrets in the active namespace. Requires buildctl in the sandbox runtime. Requires GITHUB_TOKEN for GHCR push. This MVP supports sandbox-local build contexts and public GHCR image output only.
metadata:
  author: labring
---
...
- skip builds when `mode` is `reuse-image` for legacy callers only
...
- verify the pushed GHCR container package is public before reporting success
...
- modify `.sealos/crossplane/resources.yaml`
- deploy to Sealos
- run or manage a persistent shared `buildkitd` service

For the current `brain-github-deploy` GHCR image flow, requests must use `mode=build-required`; `reuse-image` cannot satisfy the final deployment output contract.
...
`source.repo` is traceability only. It may be `labring/ShipRepo` while `image.target_image` is `ghcr.io/<token-login>/shiprepo:prepare-<sha>`. Do not rewrite the image owner back to the source repository owner.
...
| `ensure-ghcr-public.mjs` | `node ensure-ghcr-public.mjs --image <ghcr.io/owner/package:tag>` | Verify the pushed GHCR container package is publicly visible before writing a succeeded result |
