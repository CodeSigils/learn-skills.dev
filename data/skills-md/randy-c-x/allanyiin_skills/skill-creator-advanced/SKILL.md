...
9) 若目標是跨工具可攜，先決定 primary hosts 與最低支援矩陣，並把設計拆成 `skill pack`、`tool gateway`、`host adapter` 三層；若尚未有明確平台需求，先以 Agent Skills 相容 skill folder 作為單一真實來源。
10) 若 skill 會包工具、MCP 或 function calling，先定義跨 host 的最小共同契約（優先考慮保守 JSON Schema / MCP，必要時再補 OpenAPI），再檢查 tool/function 名稱、參數描述、enum 與 active tool set 是否足夠清楚。
11) 做格式、相容性與引用檢查，並把結果回填到 `references/quality_checklist.md`：
...
- 若要跨工具分發，先寫 support matrix：哪些 host 是 primary、哪些只提供薄包裝、哪些環境目前不支援；不要等到包裝階段才回頭猜。

5) Phase 3：SKILL.md architecture
- 先依任務形態選 `Tool Wrapper`、`Generator`、`Reviewer`、`Inversion` 或 `Pipeline`；不要還沒選 primary structure pattern 就直接堆更多規則。
- 決定哪些內容要放 `scripts/`、`references/`、`assets/`。
- 核心流程留在 `SKILL.md`，細節與變體移到 `references/`。
- 跨工具 skill 預設採 `skill pack × tool gateway × host adapter`：核心流程與 references 保持 host-agnostic，平台特有 manifest / plugin / marketplace 設定留在 wrapper。
...
- 若 skill 需要快取、索引、登入狀態或安裝產物，不要寫回 skill folder；改放工作區或平台提供的持久路徑。

6) Phase 4：Compatibility / trust / install audit
- 先跑 `format_check.py` 與 `quick_validate.py` 修掉結構問題。
- `quick_validate.py` 現在是最小合規 gate 的相容 wrapper；核心 frontmatter 驗證邏輯已和 `format_check.py` 共用，避免規則漂移。
- 再跑 `audit_openclaw_frontmatter.py`、`audit_skill_references.py` 與 `audit_unreferenced_files.py`，確認 frontmatter surface、本地引用路徑與未被引用檔案都可解釋。
...
- 若目標是跨工具可攜，檢查 host adapter 是否只是薄包裝而不是複製 SKILL 內容；避免 wrapper 漂移成多份真相。
- 把 tool gateway 的 transport、auth、timeout、retry、idempotency 與 approval gate 一併盤點；不要把這些非功能需求散落在各 host prompt 裡。
...
- 若是跨工具 skill，至少對 primary hosts 做 routing smoke tests，確認同一份 description / negative triggers 不會在不同 host 明顯漂移。
...
- 跨工具 benchmark 至少保留 `host × workflow` 基本矩陣；若同一 skill 在不同 wrapper 上行為不同，要分 host 記錄 benchmark 與 regression gates。
...
- 若同時維護多個 host wrapper，README / marketplace / registry 要明寫支援 host、授權方式、approval 邊界與安裝差異。
...
- `references/skill-structure-patterns.md`
- `references/cross-tool-portability.md`
...
- 操作性指令優先用明確動詞開頭，例如「先檢查」「若失敗就停止並回報」。
...
- `scripts/quick_validate.py`：最小合規驗證的相容 wrapper；和 `format_check.py` 共用同一套核心 frontmatter 規則。
- `scripts/audit_openclaw_frontmatter.py`：檢查 OpenClaw frontmatter surface，例如單行 JSON `metadata`。
- `scripts/audit_skill_references.py`：檢查 `SKILL.md` 與 `references/*.md` 中引用的本地路徑在單獨打包後仍然存在。
- `scripts/audit_unreferenced_files.py`：檢查 `scripts/`、`references/`、`assets/` 中是否存在未被 `SKILL.md` 或 `references/*.md` 參考到的檔案。
