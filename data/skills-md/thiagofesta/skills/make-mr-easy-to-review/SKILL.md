---
name: make-mr-easy-to-review
description: Prepare MRs for review by cleaning noisy history, improving MR descriptions, and adding reviewer guidance without changing code behavior. Use for "make this easy to review", "tidy this MR", "clean up commits", or "annotate the diff".
---

# Make MR Easy to Review

Prepare an MR so a reviewer can quickly understand the intent, important files, and risk. The default goal is reviewability without behavior changes.

## Workflow

1. Resolve the target MR from the user-provided URL or current branch.
2. Inspect commits, diff size, changed paths, generated files, and MR description.
...
```bash
glab mr view <MR> --json title,headRefName,baseRefName,state,commits
...
When code behavior should stay untouched, prefer MR description and review notes:
...
- If the MR is too large to make reviewable with notes, recommend splitting instead of polishing around the problem.
