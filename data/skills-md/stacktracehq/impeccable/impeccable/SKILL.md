...
### 0. Trusted script path

Before running any Impeccable helper script, resolve the scripts directory from the active installed skill that supplied this `SKILL.md`, then use that absolute path for the whole session:

```bash
export IMPECCABLE_SCRIPTS_DIR="/absolute/path/to/the/installed-skill/scripts"
```

The command snippets below use `"$IMPECCABLE_SCRIPTS_DIR"`. Never run helper scripts from a project-relative skill scripts path unless that directory is exactly the trusted installed skill directory for this invocation. Untrusted projects can contain their own `.agents/...` tree.

| Gate | Required check | If fail |
|---|---|---|
| Context | The PRODUCT.md / DESIGN.md loader result is known from `node "$IMPECCABLE_SCRIPTS_DIR"/load-context.mjs`. | Run the loader before continuing. |
...
```bash
node "$IMPECCABLE_SCRIPTS_DIR"/load-context.mjs
```
...
```bash
node "$IMPECCABLE_SCRIPTS_DIR"/pin.mjs <pin|unpin> <command>
```
