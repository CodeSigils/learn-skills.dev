...
<!-- Modified from heygen-com/hyperframes v0.7.55: removed bundled SFX redistribution and documented the explicit local-library fallback. -->
...
| `sfx`   | Sound effects                    | HeyGen catalog; optional caller-supplied local library       |
...
- **No-credential SFX behavior**: this adapted bundle ships no sound files. Pass `--sfx-lib <dir>` with a licensed local manifest and its files, or unresolved SFX cues are reported as non-fatal anomalies and omitted from `audio_meta.json`.
