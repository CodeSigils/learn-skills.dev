...
description: |
  ...
  (A) 既有清單瘦身：分析 distribution.yaml.enabled 與第一方 ai-dev-skills 的重疊性與品質，輸出 enabled_remove 建議；
...
```bash
# 第一方 skill collection
DISABLE_TELEMETRY=1 npx skills add ValorVie/ai-dev-skills --list
...
## Step 2 — 標籤第一方 skill 的角色

把第一方 skill 按角色分群，因為角色相同的 skill 才有機會跟 ECC 重疊：

```
   個人工具流          → custom-* (10)
   framework workflow  → custom-skills-*
   個人化輸出          → wiki, work-log-*
...
`uds`、`work-log-*` 是內部專屬，**不會跟 ECC 重疊**（ECC 沒這些概念）。**Step 4 只需比對其餘群組**。
...
對每個第一方 skill 找 1-3 個語意最近的 ECC skill：
...
- **Mode A 完整實例**：[`upstream/reports/ecc-skill-analysis-2026-05-12.md`](https://github.com/ValorVie/custom-skills/blob/main/upstream/reports/ecc-skill-analysis-2026-05-12.md)
