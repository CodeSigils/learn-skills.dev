...
> **SDK version: v0.6.7** — always use `go get github.com/TeneoProtocolAI/teneo-agent-sdk@v0.6.7`. If `go.mod` references an older version, update before building.
...
Ask the user what kind of agent they want to build — what it does, what commands it should have, and how it should be priced. If the user already described this, proceed directly to Step 0.
...
If the command fails or the version is below 1.24, install Go 1.24+ from the official site: https://go.dev/dl — follow the instructions for your platform (macOS, Linux, or Windows).
...
Do not proceed to the next step until `go version` succeeds.
...
Generate a 64-character hex key for the agent's wallet identity:

```bash
openssl rand -hex 32
```

No `0x` prefix. The SDK derives the wallet address from this key automatically. Store it securely — if lost, the agent cannot be recovered.
...
Secure the file and ensure it won't be committed:
...
By setting `ACCEPT_EULA=true` you accept the [EULA](https://cdn.teneo.pro/Teneo_Agent_SDK_End_User_License_Agreement_(EULA)_v1_1_0.pdf) and [Public Deployment Rules](https://cdn.teneo.pro/Teneo_Agent_SDK_Public_Deployment_Rules_v1_0_0.pdf).
