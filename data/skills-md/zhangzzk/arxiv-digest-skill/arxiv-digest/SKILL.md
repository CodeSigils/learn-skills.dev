...
- use `scripts/arxiv_fetch.py` as the primary fetch path.
- the script talks to arXiv's RSS feed (`today`), Atom API (date ranges), and HTML listings (last-resort fallback) in that order, with built-in 429/503 retry, DNS fallback, and dedup.
- only use web search if the script reports failure or the user explicitly asks for it.

Period handling — pass these values to `--period`:
- `today`: papers announced on the current UTC arXiv cycle (uses RSS feed).
- `week` / `7d`: last 7 days (uses Atom API).
- `month` / `30d`: last 30 days.
- `Nd`: last `N` days.
- `YYYY-MM-DD`, `YYYY-MM`, `YYYY-MM-DD:YYYY-MM-DD`: exact day/month/range.
- `recent`: latest available postings (uses HTML listing directly).
...
python3 scripts/arxiv_fetch.py \
  ...
  --period today \
  --output ~/.local/share/arxiv-digest/fetch_<period>.json
```
- `--categories` overrides the categories list from prefs.
- `--include-replacements` keeps replacement entries (default: new + cross-list only).
- `--fast` raises page size, lowers delays — use for backfills, not for `today`.
- `--api-delay N` for being kinder to arXiv after a 429.

Exclusion policy (required):
- exclude replacement entries from digest candidates (script does this by default).
- exclude cross-listed entries from digest candidates.
- keep canonical new submissions only.
- the script already dedups by arXiv ID across categories.

Latency and quality guardrail:
- do one fetch pass, one normalization pass, and one ranking pass.
- if the script returns zero papers and didn't log an error, the window is genuinely empty — do not retry with a different fetch path expecting different data.
- if the script reports failure (non-zero exit or empty result with errors in stderr), fall back to web search (see below).

Web-search fallback (only when script fails):
- run focused queries per category and time window:
  - `site:arxiv.org/list/{category}/new`
  - `site:arxiv.org/list/{category}/recent`
  - `site:arxiv.org abs {topic keyword} {date window}`
- for each candidate, resolve and verify: arXiv ID, full title, authors, abstract, posting date, category.
- if web search also fails, report the failure to the user — do not fabricate listings.
...
```
- then pass the returned date range as the script's `--period YYYY-MM-DD:YYYY-MM-DD`.

Collection targets by period (expected after script fetch):
...
- if the script logged warnings for failed chunks, report it and mark digest as partial.
- verify each selected entry has a valid arXiv ID before ranking output.
...
- Weekend/holiday: if `--period today` returns empty, try `--period recent` or a `--period Nd` window.
- Too many papers: enforce period caps; do not dump unbounded lists.
- Too few papers: broaden categories or use `--period recent`.
- Script reports network failure (429 retries exhausted, DNS, all fallbacks dead): fall back to web search; if that also fails, report to user — do not fabricate listings.
