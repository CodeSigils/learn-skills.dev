...
**⚠ CRITICAL ANTI-PATTERN: Writing a single draft for `full` tier is a PIPELINE VIOLATION.** In V7 runs, context compaction caused the orchestrator to forget this step's procedure and write a single draft instead of spawning 3 sub-orchestrators. V8 fixes this by loading this skill fresh at the moment it's needed. **If you find yourself about to write `$FINAL_REPORT_PATH` directly without spawning 3 `hyperresearch-draft-orchestrator` subagents, STOP. Re-read this skill. Spawn the sub-orchestrators.** (Light tier is the ONE exception — see "Light tier" section below.)

**Tier gate:** Runs for ALL tiers. For `light` tier: write a single draft directly to `$FINAL_REPORT_PATH` and skip ahead to step 15 (polish). For `full`: run the triple-draft ensemble below — step 11 (synthesizer) will turn the 3 drafts into the final report.
...
Read these inputs:
- `$ARTIFACT_DIR/scaffold.md` — vault_tag, modality, wrapper requirements
- `$ARTIFACT_DIR/prompt-decomposition.json` — atomic items, required_section_headings, response_format, citation_style, pipeline_tier
- `$TEMP_DIR/evidence-digest.md` — top claims + verbatim quotes — PRIMARY EVIDENCE LAYER (full only; absent for light)
- `$ARTIFACT_DIR/comparisons.md` (full tier) — cross-locus tensions
- `$TEMP_DIR/source-tensions.json` (full tier) — expert disagreements
- `$TEMP_DIR/coverage-gaps.md` (if exists) — items with weak source coverage
...
Read `response_format` and `citation_style` from `$ARTIFACT_DIR/prompt-decomposition.json`. These control the draft shape:
...
**Light tier writes a single draft directly to `$FINAL_REPORT_PATH`.** No subagents, no triple-draft ensemble, no synthesizer.
...
2. **Honor the structural contract.**
   - Use the literal H2 headings from `required_section_headings` in `$ARTIFACT_DIR/prompt-decomposition.json`, in order.
...
5. **Exit and route.** Once `$FINAL_REPORT_PATH` is written, return to the entry skill and invoke `Skill(skill: "hyperresearch-15-polish")`. Light tier skips steps 11–14 entirely.
...
Write the 3 angle assignments to `$TEMP_DIR/draft-angles.md` (for the run log). Each angle: 2-3 sentences describing the analytical direction.
...
5. **Write each list to disk** so the spawn template can reference it:
   - `$TEMP_DIR/draft-a-source-list.md`
   - `$TEMP_DIR/draft-b-source-list.md`
   - `$TEMP_DIR/draft-c-source-list.md`
...
prompt: |
  ...
  - output_path: $TEMP_DIR/draft-a.md (or draft-b.md or draft-c.md)
  - analytical_angle: "<the 2-3 sentence angle assignment>"
  - must_read_note_ids: [<paste the IDs from $TEMP_DIR/draft-<x>-source-list.md, e.g. 30-50 IDs>]
  - decomposition_path: $ARTIFACT_DIR/prompt-decomposition.json
  - evidence_digest_path: $TEMP_DIR/evidence-digest.md
  - comparisons_path: $ARTIFACT_DIR/comparisons.md
  - source_tensions_path: $TEMP_DIR/source-tensions.json
...
**CRITICAL: never emit bare text while the 3 sub-orchestrators are running.** They will take 5-15 minutes each. Use this time to think — append notes to `$TEMP_DIR/orchestrator-notes.md` about the synthesis you'll plan in step 11: what's the strongest thesis emerging across angles? Which atomic items will be contentious? What argumentative beats must the final draft commit to? One vault count check per minute max. Write your thoughts, don't just poll.
...
**Light tier:**
- `$FINAL_REPORT_PATH` exists, hits the length target from step 10.0, follows `required_section_headings`, and respects `citation_style`.

**Standard / full tier:**
- All three drafts exist at `$TEMP_DIR/draft-{a,b,c}.md`
- Each draft has non-trivial length (1000+ chars argumentative, 500+ structured)
- Sub-orchestrator report-backs are captured (you can paraphrase them in `$TEMP_DIR/orchestrator-notes.md` for the synthesis plan you'll write in step 11)
...
- **light tier:** You already wrote `$FINAL_REPORT_PATH` directly. Skip steps 11-14 (no synthesis, no critics, no patcher) and invoke `Skill(skill: "hyperresearch-15-polish")`.
