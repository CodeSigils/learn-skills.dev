...
**Tier gate:** SKIP entirely for `light` tier — light tier wrote `$FINAL_REPORT_PATH` directly in step 10 and proceeds straight to step 15 (polish). For `full`: run as documented below.

**Goal:** turn the 3 angle-specific drafts from step 10 into ONE integrated final report at `$FINAL_REPORT_PATH`. The orchestrator preps the strategic brief; the synthesizer subagent writes the report in two passes (rough integrated draft, then voice/redundancy/length cleanup).
...
- `$ARTIFACT_DIR/prompt-decomposition.json` — atomic items, required_section_headings, response_format, citation_style
- `$TEMP_DIR/draft-a.md`, `$TEMP_DIR/draft-b.md`, `$TEMP_DIR/draft-c.md` — the 3 angle-specific drafts from step 10
- `$ARTIFACT_DIR/comparisons.md` (full tier) — cross-locus tensions
- `$TEMP_DIR/source-tensions.json` (full tier) — expert disagreements
- `$TEMP_DIR/evidence-digest.md` — load-bearing claims with verbatim quotes
- `$QUERY_FILE_PATH` — canonical research query (GOSPEL)

---

## Step 11.1 — Read all 3 drafts in full

1. **Read each draft in full** from `$TEMP_DIR/draft-{a,b,c}.md`. Don't skim — actually read. Hold them in context.
...
3. Decide which side is correct. Write the verdict to `$TEMP_DIR/synthesis-conflicts.md`:
   ```markdown
...
Write `$TEMP_DIR/synthesis-plan.md`. This is your strategic brief for the synthesizer:
...
Write `$TEMP_DIR/synthesis-outline.md`. This is the per-section contract — 1-2 sentences per H2 naming what evidence and argument lives there:
...
- `$TEMP_DIR/synthesis-plan.md` — must include the core thesis and at least one per-section commitment
- `$TEMP_DIR/synthesis-outline.md` — must include one outline entry per H2 in the planned structure
- `$TEMP_DIR/synthesis-conflicts.md` — exists (may say "no conflicts found")
- `$TEMP_DIR/draft-{a,b,c}.md` — all three exist
...
prompt: |
  ...
  - draft_paths: [$TEMP_DIR/draft-a.md, $TEMP_DIR/draft-b.md, $TEMP_DIR/draft-c.md]
  - synthesis_plan_path: $TEMP_DIR/synthesis-plan.md
  - synthesis_outline_path: $TEMP_DIR/synthesis-outline.md
  - synthesis_conflicts_path: $TEMP_DIR/synthesis-conflicts.md
  ...
  - pass1_output_path: $TEMP_DIR/synthesis-pass1.md
  - final_output_path: $FINAL_REPORT_PATH
  - response_format: "<short|structured|argumentative>"
...
**CRITICAL: never emit bare text while the synthesizer is running.** It will take 5-15 minutes (two passes). Use the wait time to think — append notes to `$TEMP_DIR/orchestrator-notes.md` about what you'll watch for in step 12 (the critics) based on the synthesis plan you just wrote.
...
1. **Confirm both files exist:**
   - `$TEMP_DIR/synthesis-pass1.md` (pass 1, rough integrated)
   - `$FINAL_REPORT_PATH` (pass 2, final)
...
If a sanity check fails, hand-craft an Edit on `$FINAL_REPORT_PATH` yourself to fix it. Do NOT re-spawn the synthesizer — that's regeneration, which violates the patch-not-regenerate invariant once we have a final draft.
...
- `$FINAL_REPORT_PATH` exists, length in target range
- `$TEMP_DIR/synthesis-pass1.md` exists (debugging artifact)
