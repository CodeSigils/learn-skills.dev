...
Read these inputs:
- `$ARTIFACT_DIR/scaffold.md` — vault_tag (in Run config), modality
- `$ARTIFACT_DIR/prompt-decomposition.json` — atomic items, sub_questions, entities, pipeline_tier
- `$TEMP_DIR/coverage-matrix.md` — verbatim query phrases mapped to atomic items
...
3. **Write the combined search plan to `$TEMP_DIR/search-plan.md`** — a table with a `Lens` column:
   ```markdown
...
4. **Search gap check.** Cross-check the search plan against `$TEMP_DIR/coverage-matrix.md`. For every row in the coverage matrix, verify at least one search in the plan targets that query phrase's atomic item. Re-read the verbatim query and check: is there any significant topic, entity, or category in the query that has ZERO rows in the search plan?
...
Write to `$TEMP_DIR/scored-urls.md`.
...
prompt: |
  ...
  > {{paste contents of $QUERY_FILE_PATH}}
...
**Use wait time to think.** While subagents are working, write evolving thoughts to `$TEMP_DIR/orchestrator-notes.md`:
...
4. **Write coverage report** to `$TEMP_DIR/coverage-gaps.md`:
...
1. **Collect all claims.** Read `$CLAIMS_DIR/<note-id>.json` for every non-deprecated note tagged `<vault_tag>`. If no claim files exist, skip this step.
...
5. **Write `$TEMP_DIR/redundancy-audit.md`** — clusters, adjusted coverage counts, atomic items dropping below 2 → flag for Wave 3.
...
prompt: |
  ...
  - output_path: $TEMP_DIR/source-analysis-<source_note_id>.md
  - vault_tag: <vault_tag>
```
...
- `$TEMP_DIR/coverage-gaps.md` written
- (For std/full): `$TEMP_DIR/redundancy-audit.md` written if any claim files existed
