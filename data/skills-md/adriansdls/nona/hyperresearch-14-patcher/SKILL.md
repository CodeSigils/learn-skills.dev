...
- `$FINAL_REPORT_PATH` — the synthesized final report from step 11
- All `$ARTIFACT_DIR/critic-findings-*.json` files (count depends on tier)
- `$TEMP_DIR/evidence-digest.md` — patcher's primary citation source
...
python -c "
import json, pathlib
files = ['$ARTIFACT_DIR/critic-findings-dialectic.json','$ARTIFACT_DIR/critic-findings-depth.json','$ARTIFACT_DIR/critic-findings-width.json','$ARTIFACT_DIR/critic-findings-instruction.json']
total = sum(len(json.loads(pathlib.Path(f).read_text()).get('findings',[])) for f in files if pathlib.Path(f).exists())
pathlib.Path('$ARTIFACT_DIR/patch-log.json').write_text(json.dumps({'total_findings': total, 'applied': [], 'skipped': [{'reason': 'patcher-skipped-by-invoker'}], 'conflicts': [], 'orchestrator_escalated': []}))
"
```
...
```bash
echo '{"total_findings": 0, "applied": [], "skipped": [], "conflicts": [], "orchestrator_escalated": []}' > $ARTIFACT_DIR/patch-log.json
```
...
prompt: |
  ...
  - findings_paths: [
      $ARTIFACT_DIR/critic-findings-dialectic.json,    (full tier only)
      $ARTIFACT_DIR/critic-findings-depth.json,        (full tier only)
      $ARTIFACT_DIR/critic-findings-width.json,
      $ARTIFACT_DIR/critic-findings-instruction.json
    ]
  - patch_log_path: $ARTIFACT_DIR/patch-log.json   (already stubbed)
  - evidence_digest_path: $TEMP_DIR/evidence-digest.md
```

The patcher's job:
- Apply each finding's patch as an Edit on the draft file
- Populate the pre-stubbed patch log via Edit on `$ARTIFACT_DIR/patch-log.json`
...
- **Is the patch log still the empty stub?** If yes, the patcher failed to log — its Task result will contain the real log inline. Read the Task result, parse out the JSON, and write it to `$ARTIFACT_DIR/patch-log.json` yourself via Bash so downstream lint rules see it.
...
2. Apply the restructure via hand-written Edit calls on `$FINAL_REPORT_PATH`. You have Write and Edit access — the tool lock applies only to the patcher and polish auditor subagents.
...
- `$ARTIFACT_DIR/patch-log.json` exists with `total_findings` set and at least one of `applied` / `skipped` / `conflicts` populated
...
- `$FINAL_REPORT_PATH` has been edited (or no edits needed if findings were trivial)
