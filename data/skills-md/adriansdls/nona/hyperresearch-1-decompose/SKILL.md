...
1. **Re-read the canonical research query** end to end (`$QUERY_FILE_PATH`).
...
4. **Write `$ARTIFACT_DIR/prompt-decomposition.json`:**
...
8. **Coverage matrix self-audit.** Re-read the verbatim query. Walk through it phrase by phrase and extract every **significant noun phrase, proper noun, technical term, and category name**. For each:
   ...
   Write the matrix to `$TEMP_DIR/coverage-matrix.md`:
...
9. **Update the scaffold.** Append a "Tier rationale" subsection to `$ARTIFACT_DIR/scaffold.md` with a 2-3 sentence justification for the tier classification.

---

## Exit criterion

- `$ARTIFACT_DIR/prompt-decomposition.json` exists, is valid JSON, every atomic item traces to the research_query
- `pipeline_tier` + `response_format` + `citation_style` are all set
- `$TEMP_DIR/coverage-matrix.md` exists with **zero `Gap? = YES` rows**
- `$ARTIFACT_DIR/scaffold.md` includes a Tier rationale subsection

---

## Next step

Return to the entry skill (`hyperresearch`). Read `$ARTIFACT_DIR/prompt-decomposition.json` to learn the tier, then invoke step 2:
