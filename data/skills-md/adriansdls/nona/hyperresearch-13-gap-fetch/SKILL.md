...
- All `$ARTIFACT_DIR/critic-findings-*.json` files (which exist depends on tier)
...
4. **Run targeted fetch wave.** For each gap, generate 2-3 search queries and collect URLs. Spawn **2-4 fetchers** with the gap-filling URLs.
   ...
   **Spawn template:**
   ...
   Each fetcher: fetches, quality-checks, summarizes, extracts claims (same procedure as step 2). Tags notes with `vault_tag` + `post-critic-fill`. Writes claims to `$CLAIMS_DIR/<note-id>.json`.

5. **Update evidence digest.** If new claims were extracted, append them to `$TEMP_DIR/evidence-digest.md` under a new `### Post-critic gap fill` section. The patcher reads the evidence digest when looking for citation sources to insert.

6. **Log results** to `$TEMP_DIR/post-critic-fetch-log.md`:
...
- `$TEMP_DIR/post-critic-fetch-log.md` exists (even if it says "no gaps found")
