...
metadata:
  ...
  adapted_from: "vyralcontent-skills/youtube-shorts (MIT)"
  version: 0.1.0
...
`short-form-ideas`. For caption and on-screen text craft, see
`captions-and-ctas`. For TikTok or Reels, see those skills.
...
3. **Write the first 3 seconds first.** No logo, no welcome-back, no animated
   ...
   and a first line that promises a specific payoff. See `hooks` for the
