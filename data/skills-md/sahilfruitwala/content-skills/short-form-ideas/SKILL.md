...
metadata:
  ...
  adapted_from: "vyralcontent-skills/viral-short-form-ideas (MIT)"
  version: 0.1.0
...
For hook writing itself, see `hooks`. For platform algorithm
mechanics, see `tiktok-content`, `youtube-shorts`, and `instagram-reels`.
For captions, CTAs, and on-screen text, see `captions-and-ctas`. For
script structure and retention, see `shortform-script`.
