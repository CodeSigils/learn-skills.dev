...
metadata:
  ...
  adapted_from: "vyralcontent-skills/instagram-reels (MIT)"
  version: 0.1.0
...
For cross-platform hook craft, see `hooks`. For ideation systems, see
`short-form-ideas`. For deep caption work, see `captions-and-ctas`.
...
3. **Write the hook for the three-second cliff.** Concrete, specific, no
   "hey guys" intro. See `hooks` for the hook archetypes themselves;
...
6. **Caption, on-screen text, hashtags.** Write the caption as SEO + a
   ...
   → `references/caption-and-search.md` (and see also `captions-and-ctas`).
