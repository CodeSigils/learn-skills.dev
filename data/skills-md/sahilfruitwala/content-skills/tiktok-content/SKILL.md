...
metadata:
  ...
  adapted_from: "vyralcontent-skills/tiktok-content (MIT)"
  version: 0.1.0
...
For cross-platform hook archetypes, see also `hooks`. For Reels or
Shorts, see `instagram-reels` and `youtube-shorts`. For ideation,
see `short-form-ideas`. For caption craft and CTAs, see
`captions-and-ctas`.
...
6. **Captions, on-screen text, hashtags.** Front-load the first sentence (it
   ...
   not 20. Caption craft lives in `captions-and-ctas`.
