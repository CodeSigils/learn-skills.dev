...
description: "Bridge between OMO Prometheus and TKT ticket lifecycle. Generates structured review context for Prometheus after bundle close, converts Prometheus plans into TKT bundle commands, and writes review feedback into the Review Agent Inbox for automatic ticket ingestion. Load this skill when you need to: (1) generate a review prompt for a completed ticket/bundle, (2) convert a Prometheus plan.md into TKT worker tickets, (3) write review feedback back into the ticket system."
license: MIT
metadata:
...
**Output**: JSON with `goal`, `tickets[]`, `tkt_commands[]` (ready-to-execute shell commands)
...
- Categories (`visual-engineering`, `deep`, `quick`, etc.) → workflow `agent_type`
...
```skill-manifest
{
  ...
  "operations": {
    ...
    "plan-to-bundle": {
      "description": "Convert Prometheus plan markdown into TKT bundle creation commands",
      "input": { "plan_file": { "type": "string", "required": true } },
      "output": { "description": "Bundle creation commands", "fields": { "goal": "string", "tickets": "array", "tkt_commands": "array" } },
      "entrypoints": {
