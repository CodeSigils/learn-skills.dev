...
```bash
bash "<this-skill-dir>/scripts/tkt.sh" update --bundle B-001 --ticket TKT-001 --status done --summary "<result>"
```

#### `add-ticket`

```bash
bash "<this-skill-dir>/scripts/tkt.sh" add --bundle B-001 --type worker --title "<title>" --description "<desc>"
```
...
```bash
bash "<this-skill-dir>/scripts/tkt.sh" close --bundle B-001
```
...
```skill-manifest
{
  ...
  "capabilities": [
    ...
    "tkt-status", "tkt-close", "tkt-review", "tkt-list",
  ...
  "operations": {
    ...
    "create-bundle": {
      ...
      "entrypoints": {
        "unix": ["bash", "{skill_dir}/scripts/tkt.sh", "create-bundle", "--goal", "{goal}"],
    ...
    "close-bundle": {
      "description": "Close bundle (requires all worker TKTs done/failed)",
      "input": { "bundle": { "type": "string", "required": true } },
