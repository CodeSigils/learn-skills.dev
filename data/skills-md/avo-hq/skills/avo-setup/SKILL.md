...
**Explicit (Avo named):** "install Avo", "add the Avo gem", "which Avo tier / `avo` vs `avo-pro` vs `avo-advanced` vs `avo-everything`", "mount Avo at `/admin`", "put Avo on a subdomain", "set the Avo license key", "add my Gem Server Token", "`BUNDLE_PACKAGER__DEV`", "append a route inside the Avo engine".
...
### 2. Pick the tier and add it to the Gemfile

The `avo` Community gem is free and comes from rubygems.org. Paid features ship as add-on gems from the private server, grouped into bundles:

- **`avo`** — Community (free): resources, most fields, sorting, filtering, actions, associations, appearance, i18n (~70% of Avo).
- **`avo-pro`** — Dashboards + Menu Editor.
- **`avo-advanced`** — everything in `avo-pro` + Dynamic Filters.
...
A paid tier — declare the bundle inside a `source` block pointing at the private server. The bundle gem pulls `avo` in transitively, and requiring it activates every add-on it includes:

```ruby
# Gemfile
source "https://packager.dev/avo-hq/" do
  gem "avo-advanced", ">= 4.0.0"   # or avo-pro / avo-everything
end
```

You can list individual add-on gems (`avo-dashboards`, `avo-dynamic_filters`, `avo-menu_editor`, `avo-authorization`, …) inside the same block instead of a bundle if you only want specific features. Either way, the paid gems come from `packager.dev` and need the token from step 3 to `bundle install`.
...
### 3. Authenticate the private gem server (paid tiers only)
...
Community needs no key. For a paid tier, drop the key into the initializer (an env var keeps it out of source control):
...
| `source "https://packager.dev/avo-hq/"` | Gemfile block that scopes paid gems to the private server | wrap `gem "avo-advanced"` |
...
- **Bundle gems pull `avo` transitively.** `avo-advanced` / `avo-everything` depend on `avo`, so you don't list `avo` twice — but the bundle gem itself must sit inside the `packager.dev` `source` block, since that's where the private gem resolves from.
...
- The **tier** installed (`avo` Community vs a paid bundle) and, for paid, **where** the gem-server token lives (bundler config, host env var, CI secret) — never echo the token value itself.
