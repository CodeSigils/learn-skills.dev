...
```bash
bundle update --conservative avo avo-pro avo-dashboards   # …every avo* gem in the lock
```
...
Guide sections are headed by version (`## Upgrade to 3.22.0`, `## Upgrade from 3.16.2 to 3.16.3`) or by the change itself when a version only shipped one. Some are per-add-on (`## Upgrade to avo-kanban 0.1.18`). Newest is at the top — **read up, then apply down.**
