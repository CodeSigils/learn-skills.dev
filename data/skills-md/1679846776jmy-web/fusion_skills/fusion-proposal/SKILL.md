---
name: fusion-proposal
description: |
  Proposal-first scientific writing pipeline for fusion-domain research proposals. Use for composing, revising, or auditing research proposals, opening reports, research plans, and evidence-grounded scientific writing, with built-in fusion funding-structure guidance (DOE OFES, EUROfusion, and 中国科技部 MOST 聚变专项/重点研发计划). Three modes (compose/revise/hybrid) with a four-layer QA pipeline. Enforces evidence-before-prose, argument-before-sections, and contracts-before-paragraphs. 触发词：申报书、基金申请、科研写作、proposal、DOE OFES、EUROfusion、聚变专项、重点研发计划、开题报告、研究计划.
license: MIT
metadata:
  hermes:
    tags: [fusion, research, writing, proposal, revision, qa, multi-agent]
    related_skills: [fusion-writing, fusion-polishing, professor, avoid-ai-writing, docx]
---

# fusion-proposal — proposal-first 科研写作 pipeline

受 autonovel（状态机+打分）、professor（动态专家）、brainstorming（入口追问）、anti-AI-writing（语言清理）启发的科研写作状态机。**不是通用"帮我写论文"prompt。**

`fusion-proposal` 由通用 proposal 写作 skill（原 `nature-proposal-writer` / `researchwrite`）改造而来，注入聚变申报书结构知识（DOE OFES / EUROfusion / MOST 聚变专项）。需要初始化模板、
...
**工作目录**：`<outputs>/fusion-proposal/<project-slug>/`
...
| `references/doe-ofes-proposal.md` | 撰写美国 DOE OFES 聚变申报书时（结构/科学价值/团队/方法/预算/里程碑评审要点） |
| `references/eurofusion-proposal.md` | 撰写 EUROfusion 申报书时（工作任务包 WPs、JT-60SA/ITER 参与、路线图对齐） |
| `references/most-fusion-proposal.md` | 撰写中国科技部聚变专项（重点研发计划）申报书时（指南对齐、创新点写法，中文） |

## 聚变申报书结构 (Fusion Funding Structures)

当申报目标属于聚变资助体系时，先加载对应 reference 锁定结构与评审要点，再走 proposal-first 流程：

| 资助方 | Reference | 关键结构 |
|--------|-----------|---------|
| 美国 DOE OFES | `references/doe-ofes-proposal.md` | 科学价值、团队、方法、预算、里程碑 |
| EUROfusion | `references/eurofusion-proposal.md` | 工作任务包 WPs、JT-60SA/ITER 参与、路线图对齐 |
| 中国科技部（重点研发计划） | `references/most-fusion-proposal.md` | 指南对齐、创新点、技术路线、考核指标（中文） |

结构知识只做"骨架与评审要点"参考，证据、论证、契约仍由本 skill 的 foundation 五文件与四层 QA pipeline 保证。
...
```
用户: "用 fusion-proposal 审查这段 discussion，paper 挡位"
