...
name: fusion-figure
description: >-
  Create, revise, audit, and export submission-grade scientific figures for fusion-domain (magnetic/inertial confinement fusion, tokamak/stellarator/spherical-tokamak physics, diagnostics, materials, engineering) and other high-impact venues in Python (matplotlib/seaborn) or R (ggplot2/patchwork/ComplexHeatmap), including multi-panel plots, figures4papers-style work, and journal-ready SVG/PDF/TIFF outputs. Use for paper or scientific plots, manuscript data visualization, 论文配图、学术写作配图、科研绘图、科研作图、画图、作图、出图、论文图表、可视化, and fusion-specific plots such as flux-surface/equilibrium (磁面图/平衡图), radial profiles (径向剖面图), time traces (时间演化图), stability diagrams (稳定性图), H-mode transition (H-mode 转换图), divertor heat flux (偏滤器排热图), and neutron yield (中子产额图). Define the conclusion, evidence logic, data integrity, template compatibility, export needs, and reviewer risks before plotting; honor or persist the Python/R backend choice. Also use the separate OpenRouter GPT Image 2 route for explicit AI-generated graphical abstracts, mechanism diagrams, concept schematics, 论文示意图、机制示意图、图形摘要; this route skips backend choice and treats outputs as drafts. Do not use for interactive dashboards, statistics-only analysis, data cleaning, literature review, code debugging, pure photo editing, or Illustrator/Figma-first infographics without manuscript-figure intent.
---

# Fusion Figure Making — Router

This skill is split into two layers:

- A **static layer** under `static/` that holds versioned, reusable content fragments (the figure contract and default stance, plus a per-backend quick-start for Python and R, and a fusion Python quick-start).
...
1. If the current request explicitly chooses Python or R, use that backend and save it with `scripts/fusion_figure_backend.py set python` or `scripts/fusion_figure_backend.py set r`.
2. If the request provides a clearly language-specific input file/workflow, use that backend and save it.
3. Otherwise run `scripts/fusion_figure_backend.py get`. If it returns `python` or `r`, use the saved preference.
...
When `backend=python` and the figure is fusion-domain plasma work (flux surfaces, profiles, time traces, divertor, neutron yield), also load `static/fragments/backend/fusion-python.md` for the eqdsk/plasma quick-start.
...
3. Backend fragment — the exclusive Python or R quick-start and execution rule; add the fusion Python quick-start when applicable.
4. Fusion figure-type guidance — when the panel is a flux surface, profile, time trace, stability diagram, H-mode transition, divertor heat-flux, or neutron-yield plot, load `references/fusion-figure-types.md` before laying out.
...
- Fusion figure-type guidance and plasma Python tooling live as on-demand references so the generic figure workflow stays reusable while fusion knowledge stays shareable.
- This structure mirrors `fusion-writing`, `fusion-polishing`, and `fusion-reader`.
