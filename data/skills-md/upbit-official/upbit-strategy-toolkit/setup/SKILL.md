...
- **Do not recommend global installation** - do not present global CLI installation as an official path.
...
| Mode | Detection | Data root |
|---|---|---|
| clone mode | Current cwd or an ancestor has `name = "upbit-strategy-toolkit"` in `pyproject.toml` and `upbit_strategy_toolkit/` | The repo root (absolute). The wrapper pins it to the repo root regardless of your cwd, so cache/strategies/reports always land under the repo even when invoked from a subdirectory. |
| agent skills mode | Only the skill was installed with `npx skills`, and the current workspace does not contain the upbit-strategy-toolkit repo | `$HOME/.upbit-strategy-toolkit` |
...
### 3-B. Agent Skills Mode Diagnostics

When the repo is not cloned, check by running the CLI directly from the GitHub source.
...
- agent skills mode: `uv --version`, `uvx --from git+https://github.com/upbit-official/upbit-strategy-toolkit.git upbit-strategy-toolkit --help`
