...
description: 生成横向翻页网页 PPT（单 HTML 文件），含 WebGL 背景、章节幕封、数据大字报、图片网格等模板。提供三种风格：① "电子杂志 × 电子墨水"（衬线 + 流体背景 + 暖色） ② "瑞士国际主义"（无衬线 + 网格点阵 + IKB/柠檬黄/柠檬绿/安全橙高亮） ③ "华为企业汇报"（白灰底 + Huawei red + 企业总结/计划/商务汇报版式）。当用户需要制作分享 / 演讲 / 发布会风格的网页 PPT，或提到"杂志风 PPT"、"瑞士风 PPT"、"华为 PPT"、"企业汇报模板"、"Swiss Style"、"horizontal swipe deck"时使用。
---
...
### 风格 C · 华为企业汇报（Huawei Corporate）

- **白/浅灰企业汇报底 + Huawei red 主强调色 + 黑灰正文**
- **右上品牌位、左上红方块标题锚点、目录红色侧栏、红色圆形图标和箭头流程**
- 适合：公司总结、工作计划、商务汇报、部门复盘、ICT / 云 / AI 产品材料
- 模板：`assets/template-huawei.html` · 主题色：`references/themes-huawei.md` · 布局：`references/layouts-huawei.md`
- 美学锚点：Huawei 企业汇报 / 科技总结 / 红灰商务汇报视觉语言

**三种风格共享**：横向翻页（键盘 ← →、滚轮、触屏、ESC 索引）、Lucide 图标、`B` 低功耗/静态模式。
...
| # | 问题 | 为什么要问 |
|---|------|-----------|
| 1 | **风格 A / B / C?**(电子杂志风 / 瑞士国际主义风 / 华为企业汇报风) | **必须先问**,决定用哪个 template + layouts + themes 文件 |
...
| 6 | **想要哪套主题色?** | 杂志风 5 套(`themes.md`) / 瑞士风 4 套(`themes-swiss.md`) / 华为风 3 套(`themes-huawei.md`),挑一 |
...
| "华为 PPT" / "Huawei" / "企业汇报" / "公司总结" / "工作计划" / "商务汇报" / "红灰模板" | **C · 华为企业汇报风** |
| 内容是 AI 产品 / 技术 / 工程 / 数据汇报 | B 更合适 |
| 内容是公司总结 / 部门复盘 / 战略汇报 / 年度计划 | C 更合适 |
| 内容是行业观察 / 人文 / 故事 / 文化 | A 更合适 |
| 用户给了大量 KPI 数字 / 路线图 / 流程 | B 更合适(`Data Hero` 布局是瑞士风专长) |
| 用户想复刻传统企业 PPT 里的目录、章节、流程箭头、数据条形图 | C 更合适 |
...
- 图片风格必须贴合当前 deck 风格:风格 A 用"电子杂志 × 电子墨水";风格 B 用"瑞士国际主义 / Swiss Style";风格 C 用"华为企业汇报 / 白灰底 / Huawei red / 企业总结图表"
...
# 或 风格 C · 华为企业汇报风
cp "<SKILL_ROOT>/assets/template-huawei.html" "项目/XXX/ppt/index.html"
```

三个 `template*.html` 都是**完整可运行**的文件——CSS、翻页 JS、字体/图标 CDN 全已预设好,只有 `<!-- SLIDES_HERE -->` 占位符等待你填充 slide 内容。风格 A/B 有 WebGL/canvas 背景;风格 C 走静态企业汇报背景和轻量 CSS 入场。

**注意**:风格 A/B/C **不能混用**。layouts.md、layouts-swiss.md、layouts-huawei.md 的类分别只服务对应模板。一份 deck 只能选一套。
...
#### 2.2 · 选定主题色(按风格选择预设 · 不允许自定义)

本 skill **只允许从对应风格的预设里选一套**,不接受用户自定义 hex 值——颜色搭配错了画面瞬间变丑,保护美学比给自由更重要。

- 风格 A:从 `references/themes.md` 选 5 套电子墨水主题之一
- 风格 B:从 `references/themes-swiss.md` 选 4 套瑞士锚点色之一
- 风格 C:从 `references/themes-huawei.md` 选 3 套 Huawei corporate 红灰主题之一
...
- 风格 C 模板里有 `brand-lockup`、`cover-shell`、`agenda-layout`、`red-panel`、`process-step`、`bar-chart` 等
- 同名 class 在不同模板里**视觉表现完全不同**(例:风格 A 的 `h-hero` 是 Noto Serif SC 衬线,风格 B/C 的 `h-hero` 是无衬线但字号和语义不同)
...
1. **先 Read 当前用的模板**(至少读到 `<style>` 块末尾):
   ...
   - 风格 C → `assets/template-huawei.html`
...
**风格 C** → `references/layouts-huawei.md`:
...
| H06 Vertical Timeline | 年度演进 / 项目里程碑 |
| H07 Data Bars | 达成率 / 排名 / 横向数据对比 |
| H08 Process Arrow | 流程 / 路径 / 方法论 |
| H09 Image Evidence Grid | 案例图 / 产品图 / 现场照片 |
| H10 Closing | 结束页 |
| H11-H14 Report Basics | 前言框 / 图文报告 / 图片条 / 人物介绍 |
| H15-H18 Analysis & Model | KPI 看板 / 组合图表 / 中心辐射 / 斜向路线 |
...
│   ├── template-huawei.html  ← 风格 C · 华为企业汇报模板（种子文件）
...
│   └── validate-huawei-deck.mjs ← 风格 C 静态校验:登记版式、品牌位、图片 alt、占位符
└── references/
    ...
    ├── layouts-huawei.md     ← 风格 C · 36 种 Huawei corporate 企业汇报/商业计划版式骨架
    ...
    ├── themes-huawei.md      ← 风格 C · 3 套 Huawei corporate 红灰主题
...
2. Step 1 需求澄清**第一问**先确定风格 A / B / C,然后:
   ...
   - 风格 C:读 `themes-huawei.md` 帮用户选一套主题色
...
4. 读对应的 layouts 文件挑布局:
   ...
   - 风格 C → `layouts-huawei.md`;正文页使用 H01-H36,每页写 `data-layout`
...
8. 生成后根据风格运行校验:风格 B 用 `node scripts/validate-swiss-deck.mjs path/to/index.html`;风格 C 用 `node scripts/validate-huawei-deck.mjs path/to/index.html`;最后读 `checklist.md` 自检

**动效相关**:风格 A/B 模板已把 Motion One 的加载和 recipe 逻辑内嵌到底部 module script。你不需要改 JS,只需要按 `layouts.md` / `layouts-swiss.md` 的骨架在 HTML 里加 `data-anim` / `data-animate` 即可。离线演示靠 `assets/motion.min.js`,断网时自动降级为"无动画但内容可读"。风格 C 使用轻量 CSS reveal,仍保留 `data-anim`、横向翻页、ESC 索引和 `B` 静态模式。
...
> 违反其中任何一条,页面会从企业汇报变成普通红色模板。

1. **红色是组织识别,不是装饰刷子** — 普通正文页用红色做标题锚点、编号、图表和流程箭头;大面积红只留给封面、目录、章节和结尾。
2. **白灰底必须干净** — 背景以白/浅灰为主,靠信息层级和红色锚点建立秩序,不要叠复杂纹理。
3. **品牌位稳定** — 右上 `.brand-lockup` 内置 Huawei logo + HUAWEI 字标,不要每页换位置或放成巨大 logo。
...
**风格 C · 华为企业汇报风**:
- 红灰企业汇报:目录、图表、人物、流程页
- 科技集团汇报:章节页、红色信息条、高密度信息组织
- 现代科技总结:浅灰背景、柔和卡片、封面品牌视觉
