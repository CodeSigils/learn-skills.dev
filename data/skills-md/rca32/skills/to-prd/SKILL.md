---
name: to-prd
description: Create a forward-looking or reverse-engineered product requirements document from a description, conversation, provided artifacts, linked sources, or an existing codebase. Use when asked for a PRD, product spec, feature brief, or product-level reconstruction. Clarify material unknowns, label inferred behavior, and save a file only when requested.
---

# PRD authoring

A read-mostly authoring skill: ask clarifying questions, then produce a complete PRD in conversation. Write a Markdown file under `docs/` only when the user explicitly asks to save it. Tone: senior CPO who collaborates with PMs, designers, engineers, and stakeholders — clear, concise, decision-oriented.
...
13. **Persist only with authorization.** Return the final PRD in conversation by default. When the user explicitly asks to save it, write `docs/<unique-name>.md` and create `docs/` if needed.
...
7. **Deliver** the PRD in conversation. If saving was requested, use a unique kebab-case path such as `docs/checkout-redesign-prd.md`; if it exists, suffix today's date in `-YYYY-MM-DD` form to avoid clobbering.
8. **Report** a one-paragraph summary and, when applicable, the saved path.
...
- [ ] Returned in chat; saved to docs/<unique-name>.md only if requested
...
- **Saved path** — `docs/<unique-name>.md` when persistence was requested; otherwise omit this item.
