...
description: Analyze a product's public-facing value, buyers, positioning, pricing, onboarding, and product experience; generate and rank concrete personas and recommend an ICP. Use for business reviews, persona or ICP work, landing-page critique, and buyer-side strategy. Source analysis is read-only; save artifacts only when requested.
---
...
| --- | --- |
| Live URL (landing page, pricing page, docs, app marketing site) | Prefer reading from the local repo if the URL points at the current project's marketing site. Otherwise, use available web access. If web access is unavailable or fetching fails, ask the user to paste the page contents. Capture headline, subhead, hero CTA, social proof, pricing tiers, FAQ. |
...
1. **Read-only on all source materials.** Do not modify source code, copy, marketing pages, provided files, or any input the user shares. Return the review in conversation unless the user explicitly asks for saved strategy artifacts under `docs/`.
...
12. **Save only with authorization.** When the user asks for files, use the canonical filenames under `docs/` (see *Deliverables*). Don't invent alternative names. Create `docs/` if needed; if a file exists, suffix it with today's date in `-YYYY-MM-DD` form to avoid clobbering.
...
7. **Phase 6 — Deliver.** Return the result in conversation. If the user requested persistence, write the authorized deliverables under `docs/` per the table below.
...
- [ ] Deliverables returned in chat; saved under docs/ only if requested
...
When persistence is requested, save under `docs/` with these canonical filenames. Produce only the files the user asked for.
...
- **Saved paths** — every file written when persistence was requested; otherwise omit this item.
