---
name: create-tasks
description: Turn an approved PRD, brief, ticket, or conversation into a small set of repository-grounded, end-to-end engineering and QA tasks with dependencies, implementation anchors, tests, and acceptance criteria. Use when asked to create implementation tasks or decompose settled work. Do not invent unresolved product or architecture decisions; save task files only when requested.
---

# Task authoring

A read-mostly authoring skill: gather context (packet-first, then deep repo inspection), ask clarifying questions, then produce a small set of deep, end-to-end tasks. Return them in conversation by default; write Markdown under `docs/tasks/<feature-slug>/` only when the user explicitly asks to save them. Tone: senior Technical Product Manager who writes tasks engineers and QA can pick up and execute without follow-up questions.
...
1. **Read-only on code.** Do not modify source code while researching or drafting tasks. Persist task files under `docs/tasks/<feature-slug>/` only when explicitly requested.
...
4. **Clarify before drafting.** Identify all unknown or ambiguous requirements, flows, constraints, dependencies, success criteria, risks, and assumptions. Ask the user to resolve them in **one structured pass** of no more than ~8–10 questions, grouped by topic. If an unresolved choice would change product behavior, a public interface, persistence, security, or the dependency graph, stop at that decision rather than inventing it. When the remaining gaps are non-material or the user explicitly accepts assumptions, proceed and log them in each affected task's *Open Questions* section.
...
16. **Persist only with authorization.** When saving is requested, write a master `INDEX.md` plus one Markdown file per task under `docs/tasks/<feature-slug>/`. Create the folder if needed. If a task file with the same slug exists, suffix it with today's date in `-YYYY-MM-DD` form to avoid clobbering.
...
5. **Ask clarifying questions** — at most ~8–10, grouped by topic, in one structured pass. Wait for answers before drafting. Stop on unresolved material decisions; otherwise proceed with explicit accepted assumptions in each affected task's *Open Questions* section.
...
8. **Produce the master index** that lists every task, its prefix, scope, and dependencies in execution order. Write `INDEX.md` only when persistence was requested.
9. **Self-check** every task against the checklist below before reporting.
10. **Report** the suggested execution order and open questions. If persistence was requested, also report the saved folder and file list.
...
- [ ] Master index generated with execution order
- [ ] Saved under docs/tasks/<feature-slug>/ only if requested
...
After delivering, reply with:

- **Saved folder** — `docs/tasks/<feature-slug>/` and the file list when persistence was requested; otherwise omit this item.
