---
name: break-fix
description: Run an authorized adversarial end-to-end bug hunt against a local, development, or staging app, then reproduce and root-cause confirmed defects. Use for bug bashes, exploratory QA, hostile user-flow testing, or a requested regression-test-and-fix loop. Default to hunt and report unless fixes are requested; not for production, diff review, or feature delivery.
---

# Break-fix — adversarial e2e bug hunt, then root-cause and fix

Select the mode from the user's request. **Hunt mode** is the default and is read-only: drive the running app until it does something it should not, reproduce each defect, diagnose the root cause, and report. **Fix mode** requires an explicit request to fix or add regression coverage; only then encode each defect as a failing e2e test, fix it, and watch the test go green. Never commit, push, deploy, or mutate tracker state unless separately requested.

In fix mode, the regression test is not paperwork at the end — it is how you prove the bug was real and that the fix addresses it. A test written *after* a fix, that has never once failed, guards nothing.
...
5. **Fix causes, not symptoms.** In fix mode, if a double-click creates two orders, the fix is a server-side idempotency guard — disabling the button just hides it from careful users. See *Phase 4*.
6. **Red before green.** In fix mode, every regression test must be observed failing against the unfixed code, for the right reason, before you fix anything.
...
- [ ] Phase 5  Regression tests written & observed failing (red; fix mode only)
- [ ] Phase 6  Fixes applied, tests green, full suite clean (fix mode only)
- [ ] Phase 7  Report delivered (saved only when requested)
```
...
**Then check in with the user.** Present the confirmed list — severity, one-line description, repro. In hunt mode, stop at diagnosis. In fix mode, proceed with small, clearly authorized fixes; pause for anything architectural, behavior-changing, or larger than the requested hunt.
...
Keep each bug's test and fix logically paired in the diff. Do not create commits unless the user separately asks for them.

## Phase 7 — Report

Summarize the hunt in chat. If the user explicitly asks to save it, write `docs/bug-hunts/<YYYY-MM-DD>-<slug>.md` (create the folder if missing) using this structure:
