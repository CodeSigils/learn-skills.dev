...
- When an incorrect, failing, flaky, or slow behavior has an unknown cause, diagnose it before using this workflow. Use a dedicated diagnosis skill when installed; otherwise reproduce the behavior, narrow the failing path, and test competing causes directly. Diagnostic evidence may later become an input here, but first-principles reasoning is not a substitute for reproduction and falsification.
- When the open decision is a module interface or architectural seam, pass the reframed objective, retained constraints, and unresolved assumptions to a dedicated design workflow when installed. Without one, stop at explicit design criteria and alternatives; do not present an unreviewed interface as an accepted design.
...
- Pass settled product decisions to `to-prd`, implementation task definition to `create-tasks`, authorized implementation to `implement`, and completed-change assessment to `code-review` when those skills are installed. Without a companion skill, return a self-contained handoff that states the decision, evidence, constraints, unresolved assumptions, and required authorization; do not perform the next workflow implicitly.
- When the user explicitly requests a durable decision record, use their requested path or the consuming repository's documented decision-record convention. If neither resolves the destination, ask before writing. A documentation workflow may handle this when installed; otherwise write only the explicitly authorized record and identify its path.
