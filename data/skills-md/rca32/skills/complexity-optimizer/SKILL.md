...
Keep a report in the conversation by default. When the user explicitly asks to save a durable report, use their requested path or the consuming repository's documented report convention. If neither resolves the destination, ask before writing. If a documentation workflow is installed, it may handle persistence; otherwise write the authorized report directly and identify its path. Leave tracker publication and issue-backed evidence updates to the workflow that owns that external system.

## Authority and Composition

For an unexplained slow or flaky symptom, establish a reproducible cause before optimizing; use a dedicated diagnosis skill when one is installed, otherwise reproduce the symptom, narrow the failing path, and test competing causes directly. Use `code-review` rather than this skill when the request is to judge a complete change against repository Standards or an originating Spec.

This skill may make a locally authorized optimization, but it never claims or releases an issue, changes tracker state, commits, pushes, opens a pull request, merges, or publishes evidence. For issue-backed edits, obey the consuming repository's lease or ownership protocol before changing files; if the required authority cannot be verified, remain read-only and report the blocker. When a validated hotspot requires moving a public seam, use a dedicated design workflow when installed; otherwise describe the proposed seam and its compatibility impact without implementing that architectural change. Use a test-first implementation workflow when available, otherwise add or run a focused behavior test before the optimization, and leave final review to `code-review` when it is installed.

Preserve dirty and concurrently edited worktrees. Do not clean, reset, stash, or overwrite unrelated changes. If a file in the optimization scope changes during the work, re-read it and integrate only when ownership remains clear; otherwise stop with the conflict identified. If verification fails or is interrupted, report the exact completed and failed checks and do not claim the optimization is complete.
