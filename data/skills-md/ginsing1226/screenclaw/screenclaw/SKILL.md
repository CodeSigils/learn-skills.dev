...
- **窗口层级**：Windows应用有主窗口和子窗口，window_id不同。优先使用子窗口操作
- **窗口选择规则**：先尝试主窗口，若操作无效再尝试子窗口
...
- **现象**：操作完全无效，截图无变化
- **原因**：大部分操作需要子窗口才能响应
- **解决**：`get_window_list` 加 `include_children=true`，优先使用子窗口ID

#### 2. 坐标是否正确【最常见】

- **现象**：操作结果与预期不符
- **解决**：调整网格参数（减小density、增大number_size、降低opacity）或重新截图
- **截图不完整**：用 `main_window_id` 激活窗口
